...
export default function RoutesPage() {
  ...
  async function loadInitialData() {
    ...
    try {
      ...
      if (dbRoutes) {
        // Filter for Local Routes (contains 'local' in the name)
        const local = dbRoutes.filter(r => r.name.toLowerCase().includes('local'))
        setRoutes(local)
        if (local.length > 0) {
          setSelectedRoute(local[0])
        }
      }
      
      if (dbDrivers) {
        // Load active drivers dynamically
        const active = dbDrivers.filter(d => d.is_active !== false)
        setDrivers(active)
        if (active.length > 0) {
          setSelectedDriverId(active[0].id)
        }
      }
    } catch (e) {
      console.error(e)
  ...
  const handleSaveTrip = async () => {
    ...
    try {
      for (const [stopId, entry] of Object.entries(stopEntries)) {
        ...
        const invoiceNumber = `RK-LR-${Date.now().toString().slice(-4)}-${Math.floor(100 + Math.random() * 900)}`
  ...
  return (
    <div className="animate-fade-in" style={{ padding: '0.25rem' }}>
      ...
      <div className="page-header" style={{ marginBottom: '2rem' }}>
        <div>
          <h2 className="page-title">🗺️ Local Route Management</h2>
          <p className="page-subtitle">Configure stop sheets, log trip sales & expenses, and audit local route runs</p>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '1.5rem', marginBottom: '2rem' }}>
          ...
          <div className="glass-card-3d" style={{ padding: '2rem' }}>
            ...
            <div className="form-group">
              ...
              <input className="form-input" placeholder="e.g. Stop 1" value={newStopName} onChange={e => setNewStopName(e.target.value)} />
            </div>
          ...
          <div className="glass-card-3d" style={{ padding: '2rem' }}>
            ...
            <div className="form-group">
              ...
              <input className="form-input" placeholder="e.g. Local Zone A" value={newRouteName} onChange={e => setNewRouteName(e.target.value)} />
            </div>
            <div className="form-group">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: '1.5rem', marginBottom: '2rem' }}>
        ...
        <div className="glass-card-3d" style={{ padding: '2rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            >
              {savingRoute ? 'Locking...' : '💾 Lock & Route Run'}
            </button>
          </div>
        </div>
